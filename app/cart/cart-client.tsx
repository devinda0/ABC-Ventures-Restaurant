...
import type { CartWithMeal } from '@/types';
import {
...
export default function CartClient({ items }: CartClientProps) {
  ...
  const [cartItems, setCartItems] = useState<CartWithMeal[]>(items);
  ...
  const applyLocalUpdate = (update: OptimisticUpdate) => {
    setCartItems((prev) =>
      prev.map((item) =>
  ...
  const handleAdultQuantityChange = (item: CartWithMeal, delta: number) => {
    const newQuantity = Math.max(1, item.quantity + delta);
    if (newQuantity === item.quantity) return;

    applyLocalUpdate({ id: item.id, changes: { quantity: newQuantity } });
    setError(null);

    startTransition(async () => {
      try {
        await updateCartItemAction(item.id, { quantity: newQuantity });
        router.refresh();
      } catch (err) {
        console.error('Failed to update adult quantity', err);
        setError('We could not update the quantity. Please try again.');
        router.refresh();
      }
    });
  };

  const handleChildQuantityChange = (item: CartWithMeal, delta: number) => {
    const newQuantity = Math.max(0, item.childQuantity + delta);
    if (newQuantity === item.childQuantity) return;

    applyLocalUpdate({ id: item.id, changes: { childQuantity: newQuantity } });
    setError(null);

    startTransition(async () => {
      try {
        await updateCartItemAction(item.id, { childQuantity: newQuantity });
        router.refresh();
      } catch (err) {
        console.error('Failed to update child quantity', err);
        setError('We could not update the quantity. Please try again.');
        router.refresh();
      }
    });
  ...
  const handleDateChange = (item: CartWithMeal, date: string) => {
    applyLocalUpdate({ id: item.id, changes: { date } });
    setError(null);

    startTransition(async () => {
      try {
        await updateCartItemAction(item.id, { date: date || undefined });
        router.refresh();
      } catch (err) {
        console.error('Failed to update date', err);
        setError('We could not update the reservation date. Please try again.');
        router.refresh();
      }
    });
  };
  ...
  const handleRemove = (id: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        setError('We could not remove the item. Please try again.');
        router.refresh();
      }
    });
  };
  ...
  return (
    <div className="w-full flex flex-col gap-10 px-10">
      ...
      <div className="flex flex-col gap-10 w-full">
        {cartItems.map((item) => {
          const adultPrice = Number(item.meal.price);
          const childPrice = Number(item.meal.childPrice || 0);
              ...
              <div className="flex items-center justify-between w-full px-4 py-4 rounded-lg">
                ...
                <div className="px-6">
                  ...
                  <p className="text-sm text-gray-600">
                    {item.quantity} Adults x {formatCurrency(adultPrice)}
                  </p>
                  {item.childQuantity > 0 && item.meal.childPrice && (
                    <p className="text-sm text-gray-600">
                      {item.childQuantity} Children x {formatCurrency(childPrice)}
                    </p>
                  )}
                </div>
                  ...
                  <div className="text-center">
                    <div className="flex items-center gap-2 mt-2 border rounded-tl-2xl rounded-br-2xl border-primary px-4 py-2">
                      ...
                      <button
                        ...
                        disabled={isPending}
                        onClick={() => handleAdultQuantityChange(item, -1)}
                      >
                        <svg
                          width="24"
                          height="24"
                  ...
                  <div className="text-center">
                    <div className="flex items-center gap-2 mt-2 border rounded-tl-2xl rounded-br-2xl border-primary px-4 py-2">
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center hover:bg-gray-100 disabled:opacity-50"
                        disabled={isPending}
                        onClick={() => handleChildQuantityChange(item, -1)}
                      >
                        <svg
                          width="24"
                          height="24"
